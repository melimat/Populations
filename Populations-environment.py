...
environmentCapacity = int(input("Maximal capacity that nevironment can handle?: "))

global startingAmount
startingAmount = int(input("How many memmbers wolud be at the beggining?: "))
...
def plotResults(plotTime, plotAmount, envCap):
    ...
    if (extinct is True):
        print ("Cannot plot garph, your population has gone extinct.")
    else:
        plt.title("Population oscillating arround environmental capacity")
        plt.xlabel("Time")
        ...
        plt.plot(plotTime, plotAmount, label="Poulation amount")
        plt.legend()
        plt.show()
